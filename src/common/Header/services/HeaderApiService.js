...
const HeaderApiService = {
  ...
  uploadUserProfilePicture: (data, config) => ApiService.postData(HEADER_URLS.UPLOAD_PROFILE_PICTURE, data, config),
  ...
  notificationApiServices: {
    ...
    markNotificationAsReadAndDelete: notificationId =>
      ApiService.putData(`${HEADER_URLS.HEADER_NOTIFICATIONS.MARK_AS_READ_NOTIFICATION_URL}${notificationId}`),
    getNotificationAlertsDetails: id =>
      ApiService.getData(`${HEADER_URLS.HEADER_NOTIFICATIONS.NOTIFICATION_ALERTS_DETAILS}${id}`),
    markAllAsRead: () => ApiService.getData(HEADER_URLS.HEADER_NOTIFICATIONS.MARK_ALL_NOTIFICATIONS_AS_READ),
  },

  globalSearchApiServices: {
    getGlobalSearchData: params => ApiService.getData(`${HEADER_URLS.HEADER_GLOBAL_SEARCH}`, { params }),
  },
};
